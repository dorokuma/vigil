...
class VigilStorage:
    ...
    def _init_db(self):
        with self._get_conn() as conn:
            conn.executescript("""
                ...
                -- 服务器自定义名称表
                CREATE TABLE IF NOT EXISTS server_display (
                    hostname      TEXT PRIMARY KEY,
    ...
    def save_display_name(self, hostname, display_name):
        now = time.time()
        with self._get_conn() as conn:
            conn.execute("""
                INSERT INTO server_display (hostname, display_name, updated_at)
                VALUES (?, ?, ?)
                ON CONFLICT(hostname) DO UPDATE SET
                    display_name = excluded.display_name, updated_at = excluded.updated_at
            """, (hostname, display_name, now))

    def get_display_names(self):
        with self._get_conn() as conn:
            rows = conn.execute("SELECT * FROM server_display").fetchall()
            return {r["hostname"]: r["display_name"] for r in rows}

    def get_display_name(self, hostname):
        with self._get_conn() as conn:
            row = conn.execute("SELECT display_name FROM server_display WHERE hostname = ?", (hostname,)).fetchone()
