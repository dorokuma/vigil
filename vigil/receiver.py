...
class VigilHandler(BaseHTTPRequestHandler):
    ...
    def _ensure_display_name(self, hostname):
        if not self.storage:
            return
        if self.storage.get_display_name(hostname) is not None:
            return
        import socket
        try:
            ...
            if data.get("status") == "success":
                ...
                self.storage.save_display_name(hostname, flag + " " + city)
        except Exception:
            pass
    ...
    def _handle_report(self):
        try:
            ...
            if data_type == "offline":
                ...
                if self.alert_engine and VigilHandler.alert_callback:
                    for alert in self.alert_engine.check_agent_report(hostname, report_data):
                        try:
                            VigilHandler.alert_callback(alert)
                        except Exception as e:
                            logger.error("Alert push failed: %s", e)

            self._ensure_display_name(hostname)
    ...
    def _handle_api_servers(self):
        ...
        try:
            ...
            result = []
            display_names = self.storage.get_display_names() if self.storage else {}
            for hostname in all_hostnames:
                p = ping_map.get(hostname, {})
                a = agent_map.get(hostname, {})
                a_data = a.get("data", {}) if a else {}

                # 在线判断：Agent 优先（Agent 在机器本地，比远端 Pinger 可靠）
                # Pinger 判断的是"从北京能否 ping 通"，不是机器本身是否在线
                is_offline = False
                if a:
                    is_offline = bool(a.get("is_offline", False))
                elif self.pinger:
                    # 没有 Agent 的服务器才靠 Pinger
                ...
                item = {
                    ...
                    "rtt": round(p["rtt"], 1) if (p and p.get("last_ok")) else None,
                    "ping_status": ("本机" if (VigilHandler.local_hostname and hostname == VigilHandler.local_hostname)
                                   else ("网络波动" if (p and p.get("last_ok") is not None and not p.get("last_ok"))
                                   else ("未监控" if not p else None))),
    ...
    def _handle_api_server(self, hostname: str):
        ...
        try:
            ...
            if ping_data:
                ...
                rtt_val = round(ping_data["rtt"], 1) if (last_ok and ping_data.get("rtt", 0) > 0) else None
                ping_status = None
                if rtt_val is None:
                    if VigilHandler.local_hostname and hostname == VigilHandler.local_hostname:
                        ping_status = "本机"
                    elif ping_data.get("last_ok") is not None:
                        ping_status = "网络波动" if not last_ok else None
                    else:
                        ping_status = "未监控"
                result["ping"] = {
                    "rtt": rtt_val,
            ...
            # 在线判断：Agent 优先（Agent 在机器本地，比远端 Pinger 可靠）
            if agent_data and not agent_data.get("is_offline", False):
                result["online"] = True
            elif self.pinger:
                recent = self.pinger.get_recent(hostname, 6)
                if recent:
    ...
    def _handle_rename(self):
        ...
        try:
            ...
            if not hostname or not display_name:
                self._json_response(400, {"error": "hostname and display_name required"})
                return
            self.storage.save_display_name(hostname, display_name)
            logger.info("Server renamed: %s → %s", hostname, display_name)
            self._json_response(200, {"status": "ok"})
        except Exception as e:
