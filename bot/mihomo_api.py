    ...
    for attempt in range(3):
        try:
            proc = await asyncio.create_subprocess_exec(
                "ssh", "-o", "ConnectTimeout=5", "eqi12", curl_cmd,
                stdin=asyncio.subprocess.PIPE if payload is not None else None,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
            )
            stdin_data = json.dumps(payload).encode() if payload is not None else None
            stdout, stderr = await asyncio.wait_for(
                proc.communicate(input=stdin_data), timeout=15
            )
